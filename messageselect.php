...
$url = new moodle_url('/local/parents/messageselect.php', array('id' => $id));
if ($messagebody !== '') {
...
if ($deluser) {
    if (array_key_exists($id, $SESSION->emailtoparents) && array_key_exists($deluser, $SESSION->emailtoparents[$id])) {
...
if ($course->id == SITEID) {
    ...
    $strtitle = get_string('coursemessageparents', 'local_parents');
    $PAGE->set_pagelayout('incourse');
}

$link = null;
if (has_capability('moodle/course:viewparticipants', $coursecontext) || has_capability('moodle/site:viewparticipants',
        $systemcontext)) {
...
if (!empty($messagebody) && !$edit && !$deluser && ($preview || $send)) {
    require_sesskey();
    if (count($SESSION->emailtoparents[$id])) {
        if (!empty($preview)) {
...
echo '<p align="center"><a href="' . $returnto.'">' . get_string("keepsearching") . '</a>' .
        ((count($SESSION->emailtoparents[$id])) ? ', ' . get_string('usemessageformparents', 'local_parents') : '').'</p>';
